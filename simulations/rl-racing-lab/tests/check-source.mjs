...
try {
  ...
  if(!p||p.rewardVersion!==3||p.trainerVersion!==3||Math.abs(p.road-.75)>1e-9||Math.abs(p.shoulder-.3375)>1e-9||p.grass!==0||Math.abs(p.backward+1.6)>1e-9||Math.abs(p.grassTime+.18)>1e-9||Math.abs(p.temperature0-1.35)>1e-9||Math.abs(p.temperature512-1.345)>1e-9||p.reset1!==2048||p.reset8!==16384||!divisible){failed=true;console.error('\nLearning-contract execution check failed:',p)}
...
const lapTimingContext=vm.createContext({sim:{simClock:42,mode:'learn',batchLapTimeTotal:0,batchLapTimeCount:0,batchBestLapTime:0}});
try{
  vm.runInContext(`${physicsSource}\nconst car={lapStartedAt:12,lastLapTime:0};const first=recordCompletedLap(car);sim.simClock=50;sim.mode='race';const second=recordCompletedLap(car);globalThis.__lapTimingProbe={first,second,start:car.lapStartedAt,last:car.lastLapTime,total:sim.batchLapTimeTotal,count:sim.batchLapTimeCount,best:sim.batchBestLapTime};`,lapTimingContext);
  const p=lapTimingContext.__lapTimingProbe;if(!p||Math.abs(p.first-30)>1e-9||Math.abs(p.second-8)>1e-9||p.start!==50||p.last!==8||p.total!==30||p.count!==1||p.best!==30){failed=true;console.error('\nLap-timing execution check failed:',p)}
}catch(error){failed=true;console.error('\nLap-timing execution check threw:',error)}
