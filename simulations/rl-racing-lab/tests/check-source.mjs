...
if (!sceneSource.includes('NEURAL_CAMERA_VERTICAL_FOV') || !perceptionSource.includes('NEURAL_CAMERA_HEIGHT') || !perceptionSource.includes('NEURAL_CAMERA_LOOK_AHEAD') || !perceptionSource.includes('NEURAL_CAMERA_LOOK_HEIGHT') || !perceptionSource.includes('trackContextObservationValues(activeTrackId,activeTrackMirror,car.trackArc,trackLength)')) {
  failed = true;
  console.error('\nO5 observation wiring is incomplete: wide neural camera and memorized-track context must both feed captureObservation().');
}
...
console.log(`Source checks passed: ${jsFiles.length} JavaScript files parsed; O5 track-context observations/migrations + R5/A3 learning contract validated; vehicle dynamics and all track layouts validated; deterministic learning path guarded; shared helpers are early; no pre-declaration HTML-id/global hazards found.`);
