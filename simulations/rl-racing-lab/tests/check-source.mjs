...
if(!effectsSource.includes('function updateSkidMarks')||!effectsSource.includes('SKID_MARK_MAX_SEGMENTS=240000')||!effectsSource.includes("sim.headless&&sim.mode==='learn'")||!physicsSource.includes('updateSkidMarks(car)')||!carsSource.includes('resetSkidTrailForCar(car)')||!tracksSource.includes("clearSkidMarks==='function'")||!perceptionSource.includes('setSkidMarksVisible(false)')){failed=true;console.error('\nPresentation-effects invariant failed: persistent skid marks must be bounded, reset-safe, track-scoped, headless-safe, and excluded from neural POV capture.')}
