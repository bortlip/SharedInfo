// Reward integration, terminal/lap rules, and collision handling over the v1.0 sim-cade vehicle dynamics.
...
function recordForwardSurfaceMeters(surface,progress){
  ...
  if(surface==='road'){sim.batchForwardMeters+=progress;sim.batchRoadForwardMeters+=progress}
...
function simulateCar(car,dt){
  ...
  const before=car.trackIndex,nearBefore=nearestIndexFor(car),surfaceBefore=surfaceZone(nearBefore.distance);
  stepVehicleDynamics(car,{steer:car.actionSteer,throttle:car.actionThrottle},surfaceBefore,dt);
  const near=nearestIndexFor(car),progress=progressDelta(before,near.index),surface=surfaceZone(near.distance),onRoad=surface==='road';
  ...
  while(car.episodeNetProgress+1e-6>=car.nextLapProgress){car.lap++;car.nextLapProgress+=trackLength;if(sim.mode==='learn'){sim.batchLaps++;log(`Driver ${car.id+1} completed lap ${car.lap}.`)}else if(car.lap>=sim.raceLaps){markRaceFinished(car);break}}
...
function collideCars(){
  for(let i=0;i<DRIVER_COUNT;i++)for(let j=i+1;j<DRIVER_COUNT;j++){
    const a=drivers[i],b=drivers[j];if(a.pendingDone||b.pendingDone)continue;if(sim.mode==='race'&&(a.raceStatus!=='racing'||b.raceStatus!=='racing'))continue;
