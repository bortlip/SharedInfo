...
function finishTransition(car,done=false){if(!car.lastObs)return;car.rollout.push({obs:car.lastObs,action:car.lastAction,reward:car.pendingReward,value:car.lastValue,logp:car.lastLogp,done});sim.experience++;sim.totalExperience++;sim.batchReward+=car.pendingReward;car.episodeReward+=car.pendingReward;car.totalReward+=car.pendingReward;car.pendingReward=0}
function chooseAction(car){
  const obs=captureObservation(car),out=forward(obs),action=sim.mode==='race'?argmax(out.probs):sampleAction(out.probs),p=Math.max(1e-7,out.probs[action]),act=actionTable[action];car.lastObs=obs;car.lastForward=out;car.lastAction=action;car.lastValue=out.value;car.lastLogp=Math.log(p);car.lastProb=p;car.actionSteer=act.steer;car.actionThrottle=act.throttle;
