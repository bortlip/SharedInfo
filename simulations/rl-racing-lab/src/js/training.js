// Proven baseline clipped PPO backpropagation plus learning telemetry.
...
function captureBatchMetrics(avgReward){
  const metrics={update:sim.update,rewardPerExp:avgReward,forwardPerExp:sim.batchForwardMeters/Math.max(1,sim.experience),offRoadPct:sim.batchOffRoadSeconds/Math.max(.001,sim.batchDriverSeconds)*100,avgFailDistance:sim.batchEpisodes?sim.batchEpisodeDistance/sim.batchEpisodes:drivers.reduce((sum,c)=>sum+c.episodeProgress,0)/DRIVER_COUNT,resets:sim.batchResets,laps:sim.batchLaps,collisions:sim.collisions,loss:sim.lastLoss,steer:[...sim.batchSteerCounts],throttle:[...sim.batchThrottleCounts]};
