...
let track=[],tangents=[],normals=[],segLen=[],trackDistance=[],trackLength=0,avgSeg=1,TRACK_N=0,finishIndex=2,activeTrackId='mixed',trackGroup=null,treeColliders=[];
let trackBounds={minX:-90,maxX:90,minZ:-90,maxZ:90,minY:0,maxY:0,centerX:0,centerZ:0,spanX:180,spanZ:180};
function makeTrackPoints(id){return buildTrackSamples(id).map(p=>new THREE.Vector3(p.x,p.y,p.z))}
...
function buildTrack(id='mixed'){
  activeTrackId=id in TRACK_DEFS?id:'mixed';track=makeTrackPoints(activeTrackId);TRACK_N=track.length;tangents=[];normals=[];segLen=[];trackDistance=[];trackLength=0;
  ...
  const dashEvery=trackStepsForDistance(activeTrackId==='mixed'||activeTrackId==='reverse'?14:16),treeEvery=trackStepsForDistance(activeTrackId==='grandprix'?48:activeTrackId==='sweepers'?44:38),treeOffset=3.2;
