...
function trackSurfaceAtPoint(x,z,indexHint=0){let bestDistance=Infinity,bestY=0;const center=mod(Math.trunc(Number(indexHint)||0),Math.max(1,TRACK_N));for(let delta=-18;delta<=18;delta++){const i=mod(center+delta,TRACK_N),j=mod(i+1,TRACK_N),a=track[i],b=track[j],vx=b.x-a.x,vz=b.z-a.z,lenSq=vx*vx+vz*vz;if(lenSq<=1e-9)continue;const u=clamp(((x-a.x)*vx+(z-a.z)*vz)/lenSq,0,1),px=a.x+vx*u,pz=a.z+vz*u,distance=Math.hypot(x-px,z-pz);if(distance<bestDistance){bestDistance=distance;bestY=a.y+(b.y-a.y)*u}}return{surface:surfaceZone(bestDistance),distance:bestDistance,y:bestY}
