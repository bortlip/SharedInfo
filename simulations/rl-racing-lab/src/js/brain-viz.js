...
function drawNetworkActivity(canvas,car){
  ...
  const auxIndices=Array.from({length:POLICY_AUX_INPUT_COUNT},(_,i)=>VISUAL_INPUTS+i),inputIndices=sampledNodeIndices(INPUTS,Math.min(34,POLICY_AUX_INPUT_COUNT+12),auxIndices);
  ...
  layers.forEach((layer,li)=>{
    ...
    layer.indices.forEach((index,ni)=>{
      ...
      if(layer.type==='input'&&index>=VISUAL_INPUTS){const aux=index-VISUAL_INPUTS,key=aux<VEHICLE_SENSE_COUNT?VEHICLE_SENSE_KEYS[aux]:TRACK_CONTEXT_KEYS[aux-VEHICLE_SENSE_COUNT]||'context';ctx.fillStyle='#c8d2da';ctx.font='7px system-ui';ctx.textAlign='right';ctx.fillText(key,x-9,y+3);ctx.textAlign='center';ctx.font='10px system-ui'}
    });
  });
