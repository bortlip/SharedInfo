...
const SKID_MARK_SCRUB_THRESHOLD=.08,SKID_MARK_MIN_SPEED=5,SKID_MARK_SAMPLE_DISTANCE=.55,SKID_MARK_MAX_SEGMENTS=240000,SKID_MARK_CHUNK_SEGMENTS=2048;
...
function ensureSkidMarkMaterial(){if(!skidMarkMaterial)skidMarkMaterial=new THREE.LineBasicMaterial({vertexColors:true,transparent:true,opacity:.96,depthWrite:false});return skidMarkMaterial}
function newSkidMarkChunk(){const geometry=new THREE.BufferGeometry(),positions=new Float32Array(SKID_MARK_CHUNK_SEGMENTS*6),colors=new Float32Array(SKID_MARK_CHUNK_SEGMENTS*6),positionAttribute=new THREE.BufferAttribute(positions,3),colorAttribute=new THREE.BufferAttribute(colors,3);positionAttribute.setUsage(THREE.DynamicDrawUsage);colorAttribute.setUsage(THREE.DynamicDrawUsage);geometry.setAttribute('position',positionAttribute);geometry.setAttribute('color',colorAttribute);geometry.setDrawRange(0,0);const lines=new THREE.LineSegments(geometry,ensureSkidMarkMaterial());lines.frustumCulled=false;lines.renderOrder=3;ensureSkidMarksGroup().add(lines);const chunk={geometry,positions,colors,positionAttribute,colorAttribute,lines,count:0};skidMarkChunks.push(chunk);return chunk}
function appendSkidMarkSegment(a,b,strength){if(skidMarkSegmentCount>=SKID_MARK_MAX_SEGMENTS)return false;let chunk=skidMarkChunks.at(-1);if(!chunk||chunk.count>=SKID_MARK_CHUNK_SEGMENTS)chunk=newSkidMarkChunk();const offset=chunk.count*6,s=clamp(Number(strength)||0,0,1),shade=.105-.085*s;chunk.positions[offset]=a.x;chunk.positions[offset+1]=a.y;chunk.positions[offset+2]=a.z;chunk.positions[offset+3]=b.x;chunk.positions[offset+4]=b.y;chunk.positions[offset+5]=b.z;for(let i=0;i<6;i++)chunk.colors[offset+i]=shade;chunk.count++;skidMarkSegmentCount++;chunk.geometry.setDrawRange(0,chunk.count*2);chunk.positionAttribute.needsUpdate=true;chunk.colorAttribute.needsUpdate=true;return true}
function skidContactPoint(x,z,indexHint){const surface=trackSurfaceAtPoint(x,z,indexHint);return surface.surface==='grass'?null:{x,y:surface.y+.055,z}}
function skidContactPoints(car){const fx=Math.cos(car.heading),fz=Math.sin(car.heading),rx=-fz,rz=fx,rearX=car.x-fx*1.05,rearZ=car.z-fz*1.05,halfTrack=.72;return{left:skidContactPoint(rearX+rx*halfTrack,rearZ+rz*halfTrack,car.trackIndex),right:skidContactPoint(rearX-rx*halfTrack,rearZ-rz*halfTrack,car.trackIndex)}}
function skidMarkStrength(car){const speedFactor=clamp(((Number(car.speed)||0)-SKID_MARK_MIN_SPEED)/25,0,1);return clamp((Number(car.tireScrub)||0)*(.65+.35*speedFactor),0,1)}
function extendSkidTrail(state,key,current,strength){const previous=state[key];if(!current){state[key]=null;return}if(!previous){state[key]=current;return}const distance=Math.hypot(current.x-previous.x,current.z-previous.z);if(distance<SKID_MARK_SAMPLE_DISTANCE)return;if(distance>2.2){state[key]=current;return}appendSkidMarkSegment(previous,current,strength);state[key]=current}
...
function updateSkidMarks(car){const state=skidTrailStates[car?.id];if(!state)return;if(sim.headless&&sim.mode==='learn'){state.left=null;state.right=null;return}const strength=skidMarkStrength(car),marking=strength>=SKID_MARK_SCRUB_THRESHOLD&&(Number(car.speed)||0)>=SKID_MARK_MIN_SPEED;if(!marking||skidMarkSegmentCount>=SKID_MARK_MAX_SEGMENTS){state.left=null;state.right=null;return}const current=skidContactPoints(car);extendSkidTrail(state,'left',current.left,strength);extendSkidTrail(state,'right',current.right,strength)}
