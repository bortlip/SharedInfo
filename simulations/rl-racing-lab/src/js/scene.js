...
const mainCamera=new THREE.PerspectiveCamera(58,1,.1,1600),observerCameras=[],renderTargets=[];let pixelBuffer=new Uint8Array(1);
...
const sun=new THREE.DirectionalLight(0xffffff,2);sun.position.set(25,45,18);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);sun.shadow.camera.left=-520;sun.shadow.camera.right=520;sun.shadow.camera.top=520;sun.shadow.camera.bottom=-520;sun.shadow.camera.far=1000;scene.add(sun);
const ground=new THREE.Mesh(new THREE.PlaneGeometry(1200,1200),new THREE.MeshStandardMaterial({color:0x355f36,roughness:1}));ground.rotation.x=-Math.PI/2;ground.position.y=-.035;ground.receiveShadow=true;scene.add(ground);
