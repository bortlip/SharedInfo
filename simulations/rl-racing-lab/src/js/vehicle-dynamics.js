...
function stepVehicleDynamics(state,controls,surface,dt){
  ...
  state.x=(Number(state.x)||0)+state.vx*dt;state.z=(Number(state.z)||0)+state.vz*dt;state.lateralAccel=lateralAccel;state.longitudinalAccel=longitudinalAccel;state.gripUse=vehicleClamp(Math.hypot(lateralAccel,longitudinalAccel)/Math.max(.001,grip),0,1);vehicleUpdateDerived(state);const moving=vehicleClamp(state.speed/5,0,1),lateralUse=vehicleClamp(Math.abs(lateralAccel)/Math.max(.001,grip),0,1);state.tireScrub=vehicleClamp((Math.abs(state.slipAngle)/.24)*.72+lateralUse*.38,0,1)*moving;return state;
}
