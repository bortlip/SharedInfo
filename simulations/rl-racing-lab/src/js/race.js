// Evaluation races, manual track selection, and versioned checkpoint save/load.
...
function checkRaceComplete(){if(sim.mode!=='race'||sim.raceFinished||drivers.some(c=>c.raceStatus==='racing'))return;sim.raceFinished=true;sim.running=false;$('raceResult').innerHTML=`<strong>Race complete.</strong> ${raceResultsText()}`;log(`Evaluation race complete: ${raceResultsText()}`)}
function startEvaluationRace(){
  if(sim.learning)return;sim.mode='race';sim.raceLaps=Number($('raceLaps').value);sim.raceTime=0;sim.racePlaces=0;sim.raceFinished=false;discardPartialRollout('Evaluation race started; unfinished learning batch discarded.');resetGrid();primeObservations();
  drivers.forEach(c=>{c.lap=0;c.raceStatus='racing';c.finishTime=null;c.finishPlace=null;c.mesh.visible=true});drivers.forEach(chooseAction);sim.running=true;$('raceResult').innerHTML=`<strong>Official evaluation race:</strong> ${sim.raceLaps} laps. Policy frozen; deterministic actions; no backprop.`;log(`Started ${sim.raceLaps}-lap evaluation race with policy from update ${sim.update}.`);updateUI();
}
function enterLearningMode(){
  if(sim.learning)return;sim.mode='learn';sim.raceFinished=false;discardPartialRollout('Learning mode resumed with the current brain.');resetGrid();primeObservations();drivers.forEach(c=>{c.lap=0;c.mesh.visible=true});drivers.forEach(chooseAction);sim.running=true;$('raceResult').textContent='Learning mode active on the selected track. The next 512-sample batch will trigger backprop.';updateUI();
}
function changeTrackMode(value){
  if(sim.learning)return;const id=value in TRACK_DEFS?value:'mixed';sim.running=false;sim.mode='learn';sim.trackMode=id;discardPartialRollout();buildTrack(id);resetGrid();primeObservations();drivers.forEach(c=>{c.lap=0;c.totalProgress=0;c.lastRank=null;c.mesh.visible=true});$('trackSelect').value=id;
  $('raceResult').innerHTML=`<strong>Track changed:</strong> ${TRACK_DEFS[id].name}. The brain is unchanged; this is a clean transfer/generalization test until you resume learning.`;log(`Loaded track: ${TRACK_DEFS[id].name}.`);updateUI();
}
function checkpointObject(){return{format:'pov-rl-racing-lab-checkpoint',version:3,savedAt:new Date().toISOString(),architecture:{obsW:OBS_W,obsH:OBS_H,inputs:INPUTS,hidden:HIDDEN,actions:ACTIONS,observation:'single-frame'},training:{update:sim.update,totalExperience:sim.totalExperience,temperature:sim.temperature,trackMode:sim.trackMode,activeTrackId,history:sim.history},net:{w1:Array.from(net.w1),b1:Array.from(net.b1),wp:Array.from(net.wp),bp:Array.from(net.bp),wv:Array.from(net.wv),bv:net.bv}}}
function saveCheckpoint(){const data=JSON.stringify(checkpointObject()),blob=new Blob([data],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`pov-racer-v3-update-${sim.update}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);log(`Saved v3 baseline brain checkpoint at update ${sim.update}.`)}
...
function loadCheckpointData(data){
  ...
  else throw new Error('Checkpoint network architecture does not match this baseline release.');
  sim.update=Number(data.training?.update)||0;sim.totalExperience=Number(data.training?.totalExperience)||0;sim.temperature=clamp(Number(data.training?.temperature)||1.0,.72,1.35);sim.history=Array.isArray(data.training?.history)?data.training.history.slice(-120):[];sim.lastMetrics=sim.history.at(-1)||null;
  const requestedTrack=data.training?.activeTrackId in TRACK_DEFS?data.training.activeTrackId:(data.training?.trackMode in TRACK_DEFS?data.training.trackMode:'mixed');sim.trackMode=requestedTrack;sim.running=false;sim.mode='learn';sim.raceFinished=false;discardPartialRollout();buildTrack(requestedTrack);$('trackSelect').value=requestedTrack;resetGrid();primeObservations();drivers.forEach(c=>{c.lap=0;c.totalReward=0;c.totalProgress=0;c.collisions=0;c.mesh.visible=true});
  $('raceResult').innerHTML=migratedFrom?`<strong>${migratedFrom} checkpoint migrated.</strong> The brain was converted into the restored single-frame / 15-action baseline. Continue learning before comparing race performance.`:`<strong>Checkpoint loaded.</strong> Brain restored from update ${sim.update}.`;
  log(migratedFrom?`Migrated ${migratedFrom} checkpoint from update ${sim.update} into the v3 baseline architecture.`:`Loaded v3 checkpoint from update ${sim.update}.`);updateUI();
}
