// Evaluation races, manually selectable training tracks, clean-start controls, and versioned checkpoints.
function discardPartialRollout(reason=''){
  drivers.forEach(c=>{c.rollout=[];c.lastObs=null;c.pendingReward=0;c.pendingDone=false});sim.experience=0;sim.physicsAcc=0;sim.decisionAcc=0;resetBatchTelemetry();if(reason)log(reason);
}
...
function enterLearningMode(){
  if(sim.learning)return;const id=sim.trackMode in TRACK_DEFS?sim.trackMode:'mixed';sim.mode='learn';sim.raceFinished=false;discardPartialRollout(`Learning mode resumed on ${TRACK_DEFS[id].name}.`);buildTrack(id);resetGrid();primeObservations();sim.updatesSinceGridReset=0;drivers.forEach(c=>{c.lap=0;c.mesh.visible=true});drivers.forEach(chooseAction);sim.running=true;$('raceResult').textContent=`Learning active on ${TRACK_DEFS[id].name}. PPO still updates every ${BATCH_TARGET} experiences; clean-grid resets follow the selected cadence.`;updateUI();
}
function changeTrackMode(){
  if(sim.learning){$('trackSelect').value=sim.trackMode;return}const id=$('trackSelect').value in TRACK_DEFS?$('trackSelect').value:'mixed';sim.trackMode=id;sim.adaptiveResetInterval=1;sim.updatesSinceGridReset=0;
  if(sim.mode==='race'){log(`Training track set to ${TRACK_DEFS[id].name} for the next learning session; current evaluation race continues.`);return}
  const wasRunning=sim.running;sim.raceFinished=false;discardPartialRollout(`Training track changed to ${TRACK_DEFS[id].name}; partial PPO batch discarded and cars clean-started.`);buildTrack(id);resetGrid();primeObservations();drivers.forEach(c=>{c.lap=0;c.mesh.visible=true});if(wasRunning)drivers.forEach(chooseAction);sim.running=wasRunning;$('raceResult').textContent=`Training track: ${TRACK_DEFS[id].name}. Brain preserved; clean-start countdown restarted for this circuit.`;updateUI();
}
function checkpointObject(){return{format:'pov-rl-racing-lab-checkpoint',version:3,savedAt:new Date().toISOString(),architecture:{obsW:OBS_W,obsH:OBS_H,inputs:INPUTS,hidden:HIDDEN,actions:ACTIONS,observation:'single-frame'},training:{update:sim.update,totalExperience:sim.totalExperience,temperature:sim.temperature,trackMode:sim.trackMode,activeTrackId:sim.mode==='learn'?sim.trackMode:activeTrackId,resetMode:sim.resetMode,adaptiveResetInterval:sim.adaptiveResetInterval,history:sim.history,trainingWallSeconds:sim.trainingWallSeconds,trainingSimSeconds:sim.trainingSimSeconds,bestRunDistance:sim.bestRunDistance},net:{w1:Array.from(net.w1),b1:Array.from(net.b1),wp:Array.from(net.wp),bp:Array.from(net.bp),wv:Array.from(net.wv),bv:net.bv}}}
...
function loadCheckpointData(data){
  ...
  sim.update=Number(data.training?.update)||0;sim.totalExperience=Number(data.training?.totalExperience)||0;sim.temperature=clamp(Number(data.training?.temperature)||1.0,.72,1.35);sim.history=Array.isArray(data.training?.history)?data.training.history:[];sim.lastMetrics=sim.history.at(-1)||null;sim.trainingWallSeconds=Number(data.training?.trainingWallSeconds)||0;sim.trainingSimSeconds=Number(data.training?.trainingSimSeconds)||0;sim.bestRunDistance=Number(data.training?.bestRunDistance)||Math.max(0,...sim.history.map(x=>Number(x.bestRunDistance)||Number(x.batchBestRunDistance)||0));
  const trainingTrack=data.training?.trackMode in TRACK_DEFS?data.training.trackMode:'mixed',allowedResetModes=['adaptive','1','2','4','8','never'];sim.trackMode=trainingTrack;sim.resetMode=allowedResetModes.includes(String(data.training?.resetMode))?String(data.training.resetMode):'adaptive';sim.adaptiveResetInterval=[1,2,4,8].includes(Number(data.training?.adaptiveResetInterval))?Number(data.training.adaptiveResetInterval):1;sim.updatesSinceGridReset=0;sim.running=false;sim.mode='learn';sim.headless=false;sim.raceFinished=false;discardPartialRollout();buildTrack(trainingTrack);$('trackSelect').value=trainingTrack;$('resetMode').value=sim.resetMode;resetGrid();primeObservations();drivers.forEach(c=>{c.lap=0;c.totalReward=0;c.totalProgress=0;c.collisions=0;c.mesh.visible=true});
  $('raceResult').innerHTML=migratedFrom?`<strong>${migratedFrom} checkpoint migrated.</strong> Brain loaded onto ${TRACK_DEFS[trainingTrack].name}.`:`<strong>Checkpoint loaded.</strong> Brain restored from update ${sim.update} on ${TRACK_DEFS[trainingTrack].name}.`;
