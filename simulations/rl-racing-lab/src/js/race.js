// Evaluation races, locked historical control track, and versioned checkpoint save/load.
...
function startEvaluationRace(){
  if(sim.learning)return;sim.mode='race';sim.raceLaps=Number($('raceLaps').value);sim.raceTime=0;sim.racePlaces=0;sim.raceFinished=false;discardPartialRollout('Evaluation race started; unfinished learning batch discarded.');buildTrack('mixed');resetGrid();primeObservations();
  drivers.forEach(c=>{c.lap=0;c.raceStatus='racing';c.finishTime=null;c.finishPlace=null;c.mesh.visible=true});drivers.forEach(chooseAction);sim.running=true;$('raceResult').innerHTML=`<strong>Official evaluation race:</strong> ${sim.raceLaps} laps on the historical Balanced Loop. Policy frozen; deterministic actions; no backprop.`;log(`Started ${sim.raceLaps}-lap Balanced Loop evaluation race with policy from update ${sim.update}.`);updateUI();
}
function enterLearningMode(){
  if(sim.learning)return;sim.mode='learn';sim.trackMode='mixed';sim.raceFinished=false;discardPartialRollout('Learning mode resumed on the locked v0.2 Balanced Loop control.');buildTrack('mixed');resetGrid();primeObservations();drivers.forEach(c=>{c.lap=0;c.mesh.visible=true});drivers.forEach(chooseAction);sim.running=true;$('raceResult').textContent='Historical v0.2 control learning is active. Every 512-sample PPO update resets all four cars to the clean starting grid.';updateUI();
}
function changeTrackMode(value){sim.trackMode='mixed';$('trackSelect').value='mixed';$('raceResult').innerHTML='<strong>Track locked:</strong> v0.4.0 is a historical v0.2 control run on Balanced Loop only.';log('Track change ignored: historical control is locked to Balanced Loop.');updateUI()}
function checkpointObject(){return{format:'pov-rl-racing-lab-checkpoint',version:3,savedAt:new Date().toISOString(),architecture:{obsW:OBS_W,obsH:OBS_H,inputs:INPUTS,hidden:HIDDEN,actions:ACTIONS,observation:'single-frame'},training:{update:sim.update,totalExperience:sim.totalExperience,temperature:sim.temperature,trackMode:'mixed',activeTrackId:'mixed',history:sim.history},net:{w1:Array.from(net.w1),b1:Array.from(net.b1),wp:Array.from(net.wp),bp:Array.from(net.bp),wv:Array.from(net.wv),bv:net.bv}}}
function saveCheckpoint(){const data=JSON.stringify(checkpointObject()),blob=new Blob([data],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`pov-racer-v3-update-${sim.update}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);log(`Saved v3 network checkpoint at update ${sim.update}.`)}
...
function loadCheckpointData(data){
  ...
  else throw new Error('Checkpoint network architecture does not match this control release.');
  ...
  sim.trackMode='mixed';sim.running=false;sim.mode='learn';sim.raceFinished=false;discardPartialRollout();buildTrack('mixed');$('trackSelect').value='mixed';resetGrid();primeObservations();drivers.forEach(c=>{c.lap=0;c.totalReward=0;c.totalProgress=0;c.collisions=0;c.mesh.visible=true});
  $('raceResult').innerHTML=migratedFrom?`<strong>${migratedFrom} checkpoint migrated.</strong> Brain loaded onto the locked historical Balanced Loop control. Use Reset learning for a clean control run.`:`<strong>Checkpoint loaded.</strong> Brain restored from update ${sim.update} onto the locked historical Balanced Loop control.`;
  log(migratedFrom?`Migrated ${migratedFrom} checkpoint from update ${sim.update}; track forced to Balanced Loop.`:`Loaded v3 checkpoint from update ${sim.update}; track forced to Balanced Loop.`);updateUI();
}
